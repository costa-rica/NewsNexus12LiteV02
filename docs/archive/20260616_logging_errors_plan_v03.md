...
# Logging & Error Handling — Plan V03

> **Changes from V02** (resolves `docs/20260616_logging_errors_plan_v02_assessment_codex.md`;
> narrow fixes only — ownership/baseline/guardrails from V02 are unchanged):
> 1. **`PATH_TO_LOGS` must be absolute** (P1): `.env.example` uses an absolute placeholder
>    and startup validation rejects relative paths, per `LOGGING_NODE_JS_V08.md`.
> 2. **Worker test `NODE_ENV`** (P2): worker tests run with `NODE_ENV=testing` (vitest's
>    default `test` is not an allowed value); validation enforces the allowed set and an
>    invalid value fails startup.
>
> V01→V02 history (unchanged here): this cycle owns **all existing endpoints**; baseline
> distinguishes committed vs uncommitted; worker import-order/test/isolated-fatal-exit
> guardrails; portal proxy logs metadata only.
...
- **Startup env validation** with guardrails:
  ...
  - **`NODE_ENV` allowed values are `development`/`testing`/`production`** — any other value
    (including vitest's default `test`) fails validation.
  - **`PATH_TO_LOGS` must be an absolute path** — reject a relative value during validation
    (do not silently normalize), matching `LOGGING_NODE_JS_V08.md`.
  - Validation must **not** terminate the test process on import: tests set required env vars
    (incl. `NODE_ENV=testing`) before importing app/routes, and the missing/invalid-env
    fatal-exit case is tested in an **isolated child process**, not the main test worker.
...
- worker-node `.env.example` add: `NODE_ENV=development`, `NAME_APP=NewsNexusLiteWorker`,
  `PATH_TO_LOGS=/absolute/path/to/logs` (**absolute** placeholder — validation rejects
  relative paths), `LOG_MAX_SIZE=5`, `LOG_MAX_FILES=5` (keep `PORT` + scrape vars). Gitignore
  the actual log directory if it lives under the repo.
...
- worker-node: **tests run with `NODE_ENV=testing`** (set in the Vitest config/setup file or
  the `test` package script — do not rely on vitest's default `test`). Tests set the other
  required env vars (`NAME_APP`, an absolute `PATH_TO_LOGS`) before importing app/routes.
- worker-node: env-validation fatal-exit tested in an **isolated child process** — cover both
  a **missing** required var and an **invalid `NODE_ENV`** (e.g. `test`) and a **relative
  `PATH_TO_LOGS`**; representative error responses (unknown-job `NOT_FOUND` 404, `start-job`
  `VALIDATION_ERROR` 400) assert the envelope; no `console.*` remains in committed server code.
