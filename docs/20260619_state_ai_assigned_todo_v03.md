...
## Changes from V02 (resolves `..._todo_v02_assessment_codex.md`)

This revision keeps the accepted V02 structure and id-bearing merge / default-
prompt-fallback resolutions intact, and only tightens the single qualifying
concern Codex raised about rerun skip handling.

**Concern: rerun skips could overwrite completed state assignments.** V02 mixed
two unrelated "skip" situations into one path, so an implementing agent could
emit a `"skipped"` `StateAssignmentResult` for an *already-completed* rerun row
and clobber its valid `article.stateAssignment` (turning a populated cell back
into `N/A`). V03 splits them explicitly into two distinct cases that must be
handled differently:
...
article selection"). Affected tasks updated consistently: StateBar eligibility +
loop (Phase 4) and reducer/component tests (Phase 5).

## Changes carried forward from V02 (resolved `..._todo_v01_assessment_codex.md`)

These two resolutions are unchanged in V03 and remain in force:
...
- [ ] In `portal/src/state/types.ts`, replace the reserved minimal
  ...
  - Add `StateRunSummary` (`eligible`, `processed`, `assigned`, `noState`,
    ...
    - **Optional `alreadyAssigned` tally.** If the implementer wants visibility
      ...
      If the implementer chooses not to surface it, omit the field — but the
      orchestrator must still exclude those rows from processing (Phase 4). Do
      **not** fold already-completed rerun rows into the `skipped` tally, which
      counts only content-skips.
...
- [ ] `portal/src/components/state/StateBar.tsx` — action region for the State
  ...
  - Progress label + summary readout (Eligible / Assigned / No state / Failed /
    ...
    reusing the `LocationBar`/`ScrapeBar` status-card layout.
...
- [ ] `portal/src/components/layout/FlowIndicatorBar.tsx` — add a `state` clause
      ...
      failed rows visible as `N/A`. (Note: already-completed rerun rows that were
      excluded from the run still hold valid `assigned`/`no_state` assignments
      from the prior run — `canAdvance` reads from those assignments / summary
      tallies, so a rerun that only re-touches failed rows does not regress
      gating.)
...
- [ ] **Rerun skip tests (the V02→V03 fix).** Add explicit coverage proving the
...
- **Rerun behavior:** after a first run that assigns some rows, click **Start
  ...
  cells are left **unchanged** (not reset to `N/A`), while previously failed rows
  are retried.
