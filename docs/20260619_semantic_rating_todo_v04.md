...
# Nexus Semantic Rating — TODO V04
...
## Revision note (changes relative to TODO V03)

This V04 supersedes `docs/20260619_semantic_rating_todo_v03.md` and resolves the single
qualifying concern in `docs/20260619_semantic_rating_todo_v03_assessment_codex.md`. Only
the no-work-rerun processor short-circuit was added; every V03 fix and carried-forward item
remains valid and intact.

1. **All-already-scored reruns short-circuit before any model load (V04 fix).** V03 separated
   `alreadyScoredIds` from `skippedIds` and made the reducer a no-op for already-scored rows,
   but the Phase 3 processor still loaded the model and embedded keywords **before** reaching
   the terminal decision — even when there was no scoring work to do. On an all-already-scored
   rerun, `eligible.length === 0` and `alreadyScoredIds.length >= 1`: there is nothing to
   score, yet a model-load failure during this no-work rerun would mark the job `failed`,
   disabling Finish even though every prior rating is preserved — contradicting the V03
   revision note and the terminal contract (an all-already-scored rerun must end `completed`
   with Finish enabled). V04 adds an **explicit short-circuit immediately after classifying
   rows and before model/keyword loading**: if `eligible.length === 0`, the processor
   publishes final results **without loading the model or embedding keywords/articles**, then
   **completes when `alreadyScoredIds.length >= 1`** and **fails the true zero-valid run when
   `alreadyScoredIds.length === 0`** (all-skipped/no prior ratings). See
   **§ Zero-valid-rating terminal behavior** and Phase 3 (load step, short-circuit, tests).

The V03 fixes are unchanged and still required:

2. **Rerun no longer erases existing semantic ratings (V03).** Already-scored rerun rows go
   into a **separate `alreadyScoredIds` channel** (never into `skippedIds`), the reducer
   **leaves those rows untouched** (it writes no semantic field for them), and `skippedIds`
   means **only** no-usable-text rows that should display `N/A`. The terminal-status and
   Finish-gating logic counts both new scores **and** already-scored rows as valid ratings, so
   an all-already-scored rerun ends `completed` with ratings preserved and Finish still
   enabled. See **§ Result contract for scored / skipped / already-scored / failed rows**,
   **§ Terminal-stage Finish contract**, **§ Zero-valid-rating terminal behavior**, and
   Phases 1, 3, 4, 5.

3. **Terminal-stage Finish contract is explicit** (V02). `FlowIndicator` disables on
...
4. **Worker result contract carries failed rows** (V02). `SemanticResults` exposes a
...
5. **Zero-valid-rating terminal behavior is defined end to end** (V02, refined in V03/V04). The
   ...
   disabled. V04 additionally short-circuits this decision when `eligible.length === 0` so a
   no-work rerun never reaches a model load. See **§ Zero-valid-rating terminal behavior** and
   Phases 3 & 5.
...
These subsections define the cross-cutting contracts that resolve the V01, V02, and V03
...
export interface SemanticResults {
  ...
  alreadyScoredIds: string[];                       // rerun rows already carrying a rating
...
- **`skippedIds`** — rows with **no usable text** (blank after `pickArticleText`). This is
  the **only** meaning of `skippedIds`: rows that should display `N/A`. Reducer sets
...
- **`alreadyScoredIds`** — Rows skipped on rerun **because they already carry an in-memory
  semantic rating** (PRD §5.6). The reducer treats this channel as a **no-op**: it writes
  **no** semantic field for these ids and **leaves their existing `semanticRatingMax` /
  `…MaxLabel` / `…Status` / `…Error` unchanged**. They are reported only so the
  count/terminal logic and tests can reason about them; they are **not** "N/A" rows and
  **not** failures.
...
**No-work short-circuit (V04 — runs before any model load):** immediately after classifying
rows into `alreadyScoredIds` / `skippedIds` / `eligible` (and before loading the model or
embedding keywords/articles), check `eligible.length`. When `eligible.length === 0` there is
no scoring work, so the processor **publishes final results and resolves the terminal status
without ever calling `embedder.load()` or embedding any keyword/article**:

- Publish `setResults(job, { scores, skippedIds, alreadyScoredIds, failures })` first. (On
  this no-work path `scores` and `failures` are empty; `skippedIds` and `alreadyScoredIds`
  hold the classified rows.)
- `alreadyScoredIds.length >= 1` → `complete(job, summary)`. This is the all-already-scored
  rerun: it ends **`completed`** with prior ratings preserved (reducer no-op) and Finish
  enabled — and crucially it can **never** hit a model-load failure, because the model is
  never loaded on this path.
- `alreadyScoredIds.length === 0` → `fail(job, …)` with a sanitized reason (no content). This
  is the true zero-valid run (all rows blank/skipped, none carried in); the published
  `skippedIds` let the table render `N/A`, and Finish stays disabled.

**Normal path (`eligible.length >= 1`):** load the model, embed keywords once, score the
eligible rows, then always publish final results via `setResults(job, { scores, skippedIds,
alreadyScoredIds, failures })` **before** the terminal transition, **then**:

- `validRatings >= 1` → `complete(job, summary)` (success path). This includes a run where
  `scores.length === 0` but `alreadyScoredIds.length >= 1` reached the scoring path because
  some rows were eligible — it must still end **`completed`**, not `failed`, so prior ratings
  are preserved and Finish stays enabled.
...
- A **model-load failure** (initial or post-recycle, reachable only on the normal path) also
  terminates as `failed`; publish whatever results exist (already-applied per-row successes
  and the `alreadyScoredIds` list are preserved). The portal's empty/failed message covers
  both zero-valid and load-failure cases.

The portal applies `terminalJob.results` (scores + skippedIds + alreadyScoredIds + failures)
on **any** terminal status that carries results — `completed`, `cancelled`, **and**
`failed`. Because `alreadyScoredIds` is a no-op in the reducer, applying results on a
`failed` rerun terminal can **never** erase existing ratings. Finish gating still depends
...
- [ ] Create `semantic-scorer/threadEmbedder.ts` exposing the main-thread surface:
  ...
  - [ ] `recycle(reason): void` — `worker.terminate()`, reject + clear every entry in
        `pendingEmbeds` (and any in-flight `load`) with a recoverable error, and null
        `worker`/`loadPromise` so the next `load()`/`embed()` lazily spawns a fresh worker
        and reloads the model. Extend (do not duplicate) the existing `failWorker` cleanup;
        the `error`/`exit` handlers must continue to converge on the same cleanup so a
        crashed worker also respawns.
...
- [ ] Create `semantic-scorer/processor.ts` (`runSemanticJob({ job, articles, keywords,
  ...
  - [ ] **classify step (rerun-aware, PRD §5.6):** classify every input row into exactly one
  ...
  - [ ] **No-work short-circuit (V04 fix — runs BEFORE any model load).** Immediately after
        the classify step and **before** setting `modelLoading`, calling `embedder.load()`,
        or embedding any keyword/article, check `eligible.length`. When `eligible.length ===
        0`, there is no scoring work to do, so **publish final results and resolve the
        terminal status without touching the model**:
        - `setResults(job, { scores, skippedIds, alreadyScoredIds, failures })` (on this
          path `scores`/`failures` are empty; `skippedIds`/`alreadyScoredIds` hold the
          classified rows).
        - `alreadyScoredIds.length >= 1` → `complete(job, summary)`. **All-already-scored
          rerun:** ends `completed` with prior ratings preserved (reducer no-op) and Finish
          enabled. The model is **never loaded**, so this path can never become `failed` via
          a model-load failure.
        - `alreadyScoredIds.length === 0` → `fail(job, …)` with a sanitized reason (no
          content). **True zero-valid run** (all rows blank/skipped, none carried in); the
          published `skippedIds` let the table render `N/A` and Finish stays disabled.
        - **Return early** after the short-circuit terminal transition — do not fall through
          to the load/embed steps below.
  - [ ] **embed keywords once (only reached when `eligible.length >= 1`):** set
        `summary.modelLoading = 1`, `await embedder.load()`, embed **each keyword vector
        once** up front, cache them **on the main thread** for the run, then
        `modelLoading = 0`. If a keyword embed times out/hangs, apply the
  ...
  - [ ] **completion for the normal path (§ Zero-valid-rating terminal behavior):** after
        the score loop, publish final results (`setResults(job, { scores, skippedIds,
        alreadyScoredIds, failures })`) **before** the terminal transition, then compute
        `validRatings = scores.length + alreadyScoredIds.length`:
        - `validRatings >= 1` → `complete(job, summary)` (success path).
        - `validRatings === 0` (all eligible rows blank-handled/timed-out/errored/no-score
          and nothing carried in) → `fail(job, …)` with a sanitized reason (no content).
          Skipped/failed ids are already in the published results so the table can render
          `N/A`.
        - model fails to load (initial or post-recycle) → publish whatever results exist
          ...
          then `fail(job, …)`.
        (The all-already-scored rerun never reaches this block — it is handled by the
        no-work short-circuit above.)
...
- [ ] Tests:
  - [ ] `processor.test.ts` (mock embedder — no real model download):
    ...
    - [ ] **all-already-scored rerun (V03 + V04 fix):** every input row already carries a
          rating → `eligible.length === 0`, so the processor **short-circuits before any
          model load**. Assert the mock embedder's `load()` is **never called** and **no
          keyword/article embed occurs**; `scores` empty, `alreadyScoredIds` holds them all,
          `skippedIds`/`failures` empty, job terminal status **`completed`** (NOT `failed`),
          `summary.processed === 0`, `summary.alreadyScored === inputCount`,
          `summary.modelLoading === 0`. (This is the case the V02 bug marked `failed` and the
          V03 TODO still routed through a model load.)
    - [ ] **all-skipped run, no prior ratings (V04 short-circuit, zero-valid):** every row
          has no usable text and none already scored → `eligible.length === 0` and
          `alreadyScoredIds.length === 0`. Assert `load()` is **never called**, results carry
          `skippedIds`, `scores`/`alreadyScoredIds`/`failures` empty, job terminal status
          **`failed`**, `summary.processed === 0`.
    - [ ] **mixed already-scored + eligible rerun:** some rows already scored, others
          eligible → `eligible.length >= 1`, so the processor takes the normal path and
          **does** load the model. Assert `load()` is called once, already-scored ids land in
          `alreadyScoredIds` and are not re-embedded, eligible rows are scored, and the job
          ends `completed`.
    ...
    - [ ] **all-failed run** (every eligible row times out / errors / no-score, none already
          scored) → results carry `failures`, `scores`/`alreadyScoredIds` empty, job terminal
          **`failed`** (reached via the normal path, since `eligible.length >= 1`).
...
- [ ] Tests:
  ...
  - [ ] `SemanticBar` **rerun coverage (V03 + V04 fix):** an all-already-scored rerun
        ...
        values intact (no `N/A`), and Finish stays enabled. (Backed by the Phase 3
        short-circuit, which guarantees this terminal is `completed` without a model load.)
...
- [ ] Manual rerun check tied to the V03 + V04 fix:
  ...
  - [ ] **All-already-scored rerun does no model work (V04):** when the rerun's working set
        is entirely already-scored (every row carries a number), confirm the run ends
        **`completed`** with **no "Loading model…" indicator** appearing (the short-circuit
        skips the model load), ratings are preserved, and Finish stays enabled.
- [ ] Final review: confirm no durable persistence was added, no full-NewsNexus12 runtime
      ...
      never clears existing ratings, the no-work rerun short-circuits before any model load,
      and no lockfile/env/unrelated files were modified.
