...
# Foundation / Portal Shell — TODO v01
...
- [ ] Confirm the repo is sparse: verify it contains only `AGENTS.md`, `CLAUDE.md`,
...
- [ ] Read `AGENTS.md` and follow its conventions (markdown frontmatter rules, commit
      guidance). Note the Python venv rule is not relevant to this stage.
- [ ] Create the portal app under a top-level **`portal/`** directory (future-monorepo
...
- [ ] Ensure the repo `.gitignore` (or `portal/.gitignore`) excludes `node_modules/`,
      `.next/`, build output, and any local env files.

### End-of-phase checks (Phase 0)
- [ ] No build/test infra exists yet — checks deferred to Phase 1.

---

## Phase 1 — Toolchain: Next.js + TypeScript + Tailwind + lint + test harness

- [ ] Initialize the Next.js (App Router) app with React + TypeScript inside `portal/`.
...
- [ ] Configure TypeScript (`tsconfig.json`) and a `type-check` script (e.g.
      `tsc --noEmit`).
- [ ] Configure ESLint (Next.js + TypeScript rules) and a `lint` script. This lint/type
      config is inherited by all later stages — set it up cleanly now.
- [ ] Configure Tailwind CSS (config + PostCSS + `globals.css` with Tailwind layers).
- [ ] Add a test runner suitable for React component tests (e.g. Vitest or Jest +
...
- [ ] Confirm `dev`, `build`, `start`, `lint`, `type-check`, and `test` scripts exist in
      `portal/package.json`.

### End-of-phase checks (Phase 1)
- [ ] Run type-check — passes.
- [ ] Run lint — passes.
- [ ] Run the test runner (smoke test) — passes.
- [ ] Run the production build — succeeds.

---

## Phase 2 — In-memory state layer (forward-compatible, no persistence)

- [ ] Create `src/state/types.ts` with the forward-compatible `Article` type declaring
...
- [ ] Define `StageKey = "search" | "scrape" | "location" | "state" | "semantic"` and a
...
- [ ] Create `src/state/flowReducer.ts` exposing at least `setStage`, `setArticles`, and
...
- [ ] Create `src/state/FlowContext.tsx` providing the flow state + dispatch via React
      Context (Context + reducer or equivalent lightweight in-memory store). **No**
      database, no local/session storage, no files.
- [ ] Create `src/lib/pipeline.ts` as the single source of ordered stage metadata: the
...
- [ ] Create `src/components/tables/columns.tsx` as the **single source of truth** for
...
- [ ] Create presentational cell components with documented prop contracts, consumed by
      ...
      - `src/components/tables/cells/StateCell.tsx` — state text stub (populated in
        stage 5).
- [ ] Create `src/components/tables/ArticlesTable.tsx` using `@tanstack/react-table`
...
- [ ] Do not add sorting/filtering/pagination behavior beyond what the empty shell needs
...
- [ ] Copy the logo asset into `portal/public/images/logoWhiteBackground.png` (reproduce
...
- [ ] Create `src/components/layout/TopBar.tsx` — logo
      (`logoWhiteBackground.png`) + the text **"News Nexus Lite"** + the theme toggle.
      Persistent across all stages.
- [ ] Create `src/components/layout/ThemeToggle.tsx` and a theme provider — implement
      ...
      **only in memory** for the session (no storage), consistent with ephemerality.
      A default theme matching NewsNexus12 portal is fine.
- [ ] Create `src/components/layout/FlowIndicator.tsx` — a horizontal timeline of the
...
- [ ] Create `src/components/layout/SlideStage.tsx` — the slide/background transition
      **mechanism**: wraps content and animates on `currentStage` change (table/top
      bar/flow indicator give the impression of sliding right while the background
      animates left), and applies a per-stage background tint pulled from
      `lib/pipeline.ts` (lighter hue of blue in dark theme, darker hue of gray in light
      theme). Build the mechanism and per-stage color tokens so stage 2+ only flips
      `currentStage`; real cross-stage navigation is not required yet.
...
- [ ] Implement `src/app/layout.tsx` (root layout): mount the theme provider and
      `FlowContext` provider; establish the persistent top-bar frame.
- [ ] Implement `src/app/page.tsx` (the single demo page): compose, inside the
      `SlideStage` wrapper, the three persistent regions —
      1. Top bar (logo + "News Nexus Lite" + theme toggle),
      2. Flow indicator sitting on **step 1 (Search)** with the disabled "Next" button,
      3. The `ArticlesTable` with all 7 columns and an empty-state body.
- [ ] Confirm `globals.css` wires Tailwind layers + theme tokens and that the app renders
      with no console errors in `dev`.
- [ ] Add **no** server routes / API routes in this stage.
...
- [ ] Add a test asserting the column **set and order** (all 7 headers, exact labels in
      the plan/PRD order) rendered by `ArticlesTable` / `columns.tsx`.
- [ ] Add a test asserting `RatingCircle` color mapping (higher normalized score →
      greener; lower → duller/darker).
- [ ] Add a test asserting `resetFlow` clears the in-memory article set and returns the
      flow to step 1 ("search") — confirming the ephemerality mechanism.
- [ ] Add a lightweight test/interaction exercising that the `SlideStage` mounts and
      reacts to a `currentStage` change (e.g. via a dev/test trigger) without
      restructuring the table.
- [ ] Keep tests minimal but meaningful; do not test later-stage behavior that does not
      exist yet.
...
- [ ] The portal installs and runs from a clean checkout: `dev` server starts and
      `build` succeeds (repo starts sparse).
- [ ] The page renders the top bar (logo + "News Nexus Lite"), the flow indicator on
      **step 1 (Search)**, and the TanStack table showing **all 7 column headers in
      order** with an empty-state body.
- [ ] Both dark and light themes render, and the per-stage background tint token resolves
      correctly for each theme.
- [ ] The slide-transition wrapper mounts and animates on a `currentStage` change without
      restructuring the table.
- [ ] `resetFlow` clears the in-memory article set and returns the flow to step 1.
- [ ] Confirm **no persistence** is introduced: no database, no file writes for app
      state, no `localStorage` / `sessionStorage` / cookie writes for pipeline/app data.
- [ ] Re-confirm none of the OUT OF SCOPE items were implemented (no server routes, no
...
- [ ] All phases above complete and all end-of-phase checks pass.
- [ ] Every task checkbox in completed phases is checked off.
- [ ] No files outside the stage's scope were modified (this stage creates the `portal/`
...
- [ ] After type/lint, tests, and build all pass, check off all completed tasks, then
...
- [ ] Do **not** push. Do **not** start stage 2 (Search) — stop after the stage 1 commit
